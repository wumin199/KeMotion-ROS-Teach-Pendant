...
static void show_result(char *str_speech, char is_over)
{
  ...
  std::cout << "length is " << str.length() << std::endl;//3 bytes per chinese word
  printf("\rResult: [%s]\n", str_speech);
	if(is_over)
    putchar('is over\n');
...
{
  printf("\non_speech_end, g_result is [%s]\n", g_result);
  std::string str(g_result);
  std::cout<<"on_speech_end, length is " << str.length() << ", content is ["<<str << "]"<< std::endl;
...
int main(int argc, char* argv[])
{
	...
	*/
    ...
    "vad_enable=true, vad_eos=5000, vad_bos=10000";
